        ...
        while True:
            ...
            try:
                ...
                candidates = await asyncio.to_thread(
                    hybrid_retrieve, resolved_query, top_k=15, document_id=document_id
                )
                ...
                reranked = await asyncio.to_thread(
                    rerank_candidates, resolved_query, candidates, top_k=10
                )
                protected = await asyncio.to_thread(
                    protect_exact_matches, resolved_query, reranked, candidates=candidates, top_k=10
                )
                ...
                context, results = await asyncio.to_thread(
                    build_context, protected, candidates, resolved_query, 5
                )
                ...
                answer_parts = []
                for chunk in generate_answer_stream(resolved_query, (context, results)):
                ...
                if refused:
                    ...
                    citations = await asyncio.to_thread(
                        format_citations, resolved_query, answer, results
                    ) if answer else ""
                ...
                await websocket.send_json({"type": "sources", "sources": serialize_sources(results)})
                await websocket.send_json({
                    "type": "complete",
                    "query": query,
                    "resolved_query": resolved_query,
